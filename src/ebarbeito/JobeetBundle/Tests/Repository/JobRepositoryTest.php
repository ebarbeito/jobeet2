...
namespace ebarbeito\JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase {
  ...
  public function setUp() {
    ...
    $loader->loadFromDirectory(static::$kernel->locateResource('@ebarbeitoJobeetBundle/DataFixtures/ORM'));
  ...
  public function testCountActiveJobs() {
    $query = $this->em->createQuery('SELECT c FROM ebarbeitoJobeetBundle:Category c');
    $categories = $query->getResult();

    foreach ($categories as $category) {
      $query = $this->em->createQuery('SELECT COUNT(j.id) FROM ebarbeitoJobeetBundle:Job j WHERE j.category = :category AND j.expires_at > :date');
      $query->setParameter('category', $category->getId());
      $query->setParameter('date', date('Y-m-d H:i:s', time()));
      $jobs_db = $query->getSingleScalarResult();

      $jobs_rep = $this->em->getRepository('ebarbeitoJobeetBundle:Job')->countActiveJobs($category->getId());
  ...
  public function testGetActiveJobs() {
    ...
    foreach ($categories as $category) {
      $query = $this->em->createQuery('SELECT COUNT(j.id) from ebarbeitoJobeetBundle:Job j WHERE j.expires_at > :date AND j.category = :category');
      $query->setParameter('date', date('Y-m-d H:i:s', time()));
      $query->setParameter('category', $category->getId());
      $jobs_db = $query->getSingleScalarResult();

      $jobs_rep = $this->em->getRepository('ebarbeitoJobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
      ...
      if ($jobs_db > 2) {
        $jobs_rep = $this->em->getRepository('ebarbeitoJobeetBundle:Job')->getActiveJobs($category->getId(), 2);
        ...
        $jobs_rep = $this->em->getRepository('ebarbeitoJobeetBundle:Job')->getActiveJobs($category->getId(), 2, 1);
  ...
  public function testGetActiveJob() {
    $query = $this->em->createQuery('SELECT j FROM ebarbeitoJobeetBundle:Job j WHERE j.expires_at > :date');
    $query->setParameter('date', date('Y-m-d H:i:s', time()));
    $query->setMaxResults(1);
    $job_db = $query->getSingleResult();

    $job_rep = $this->em->getRepository('ebarbeitoJobeetBundle:Job')->getActiveJob($job_db->getId());
    ...
    $job_rep = $this->em->getRepository('ebarbeitoJobeetBundle:Job')->getActiveJob($job_expired->getId());
