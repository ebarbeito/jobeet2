...
namespace ebarbeito\JobeetBundle\Tests\Controller;
...
class JobControllerTest extends WebTestCase {
  ...
  public function getMostRecentProgrammingJob() {
    ...
    $query = $em->createQuery('SELECT j from ebarbeitoJobeetBundle:Job j LEFT JOIN j.category c WHERE c.slug = :slug AND j.expires_at > :date ORDER BY j.created_at DESC');
  ...
  public function getExpiredJob() {
    ...
    $query = $em->createQuery('SELECT j from ebarbeitoJobeetBundle:Job j WHERE j.expires_at < :date');
    $query->setParameter('date', date('Y-m-d H:i:s', time()));
    $query->setMaxResults(1);
  ...
  public function testIndex() {
    ...
    $this->assertEquals('ebarbeito\JobeetBundle\Controller\JobController::showAction', $client->getRequest()->attributes->get('_controller'));
